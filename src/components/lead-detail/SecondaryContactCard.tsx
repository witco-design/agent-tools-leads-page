      ...
      >
        {contact ? (
          <div className="space-y-spacing-2">
            <p className="text-text-4 font-semibold text-text-default">{contact.name}</p>
            <p className="text-text-4 text-text-secondary">{contact.relationship}</p>
            {contact.phone && <p className="text-text-4 text-text-default">{contact.phone}</p>}
            {contact.email && <p className="text-text-4 text-text-default">{contact.email}</p>}
          </div>
        ) : (
          <p className="text-text-4 font-normal text-text-muted">Unavailable</p>
        )}
      </CollapsibleCard>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="space-y-spacing-4 py-spacing-2">
            <div>
              <label className="block text-text-4 font-semibold text-text-default mb-spacing-1">Name</label>
              <input
                type="text"
                value={form.name}
            ...
            <div>
              <label className="block text-text-4 font-semibold text-text-default mb-spacing-1">Relationship</label>
              <select
            ...
            <div>
              <label className="block text-text-4 font-semibold text-text-default mb-spacing-1">Phone</label>
              <input
                type="tel"
                value={form.phone}
            ...
            <div>
              <label className="block text-text-4 font-semibold text-text-default mb-spacing-1">Email</label>
              <input
                type="email"
                value={form.email}
