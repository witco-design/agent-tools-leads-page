...
import { Code2, X, MousePointerClick, ChevronDown, Clipboard, Check } from 'lucide-react';
...
function ClassesSection({ element }: { element: HTMLElement }) {
  ...
  // Group: mapped classes by category, unmapped in "other"
...
function TypographySection({ element }: { element: HTMLElement }) {
  ...
  // Try to map the font-size to a typography token
  const sizeNum = parseFloat(cs.fontSize);
  const sizeTokenMap: Record<number, string> = {
    8: 'Text/1', 10: 'Text/2', 12: 'Text/3', 14: 'Text/4',
    16: 'Text/5', 20: 'Text/6', 24: 'Text/7', 32: 'Text/8', 60: 'Text/9',
  };
...
function computeAccessibleName(el: HTMLElement): string {
  ...
  // For elements with children, get direct text nodes only
  let text = '';
