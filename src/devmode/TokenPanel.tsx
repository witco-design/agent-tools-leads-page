...
function ColorsTab({ tokensInUse }: { tokensInUse: Set<string> }) {
  ...
  return (
    <div className="space-y-5">
      {families.map(family => (
        <div key={family}>
          <h3 className="text-xs font-semibold uppercase tracking-wide text-[#667085] mb-2">{family}</h3>
              ...
              .map(token => {
                ...
                return (
                  ...
                  >
                    ...
                    />
                    <div className="text-xs font-mono text-[#101828]">{token.name.split('.')[1]}</div>
                    <div className="text-xs font-mono text-[#667085]">{token.value}</div>
                    {inUse && <div className="text-xs text-[#3e60c9] mt-1">in use</div>}
                  </button>
                );
              })}
          </div>
...
function SpacingTab({ tokensInUse: _tokensInUse }: { tokensInUse: Set<string> }) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="text-xs font-semibold uppercase tracking-wide text-[#667085] mb-2">Spacing Scale</h3>
        <div className="space-y-2">
          {spacingTokens.map(token => (
            ...
            >
              <div className="h-4 rounded bg-[#3e60c9]" style={{ width: token.value }} />
              <div className="text-xs font-mono text-[#101828] whitespace-nowrap">{token.name}</div>
              <div className="text-xs font-mono text-[#667085]">{token.value}</div>
            </button>
          ))}
        </div>
      ...
      <div>
        <h3 className="text-xs font-semibold uppercase tracking-wide text-[#667085] mb-2">Border Radius</h3>
        <div className="grid grid-cols-3 gap-2">
...
function TypographyTab({ tokensInUse: _tokensInUse }: { tokensInUse: Set<string> }) {
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-xs font-semibold uppercase tracking-wide text-[#667085] mb-2">Type Scale</h3>
      {typoTokens.map(token => {
        ...
        return (
          ...
          >
            <div className="text-[#101828] font-medium mb-1 truncate" style={{ fontSize: size }}>
              The quick brown fox
            </div>
    ...
    <div className="space-y-2">
      ...
      {components.length === 0 ? (
        ...
        components.map(([name, count]) => (
          <div key={name} className="p-3 border border-[#E4E7EC] rounded flex items-center justify-between">
            <div>
    ...
    <div>
      ...
      {picked && (
        <div className="mt-4 p-3 border border-[#E4E7EC] rounded">
          <div className="w-full h-16 rounded border border-[#E4E7EC] mb-2" style={{ backgroundColor: picked.color }} />
          <div className="text-xs font-mono text-[#101828] mb-1">{picked.color.toUpperCase()}</div>
          {picked.token ? (
            <div className="text-xs text-[#3e60c9]">
              Nearest token: <strong>{picked.token.name}</strong> ({picked.token.value})
            </div>
          ) : (
            <div className="text-xs text-[#667085]">No matching token — consider adding to design system.</div>
          )}
        </div>
      )}
    </div>
